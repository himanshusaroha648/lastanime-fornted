...
function Home() {
        ...
        const filteredSeries = useMemo(() => {
                ...
                // Sort by latest episodes first, then remaining anime alphabetically
                if (!query.trim()) {
                        // Create a map of slug to its position in latestEpisodes (for ordering)
                        const slugOrderMap = new Map();
                        latestEpisodes.forEach((ep, index) => {
                                if (!slugOrderMap.has(ep.seriesSlug)) {
                                        slugOrderMap.set(ep.seriesSlug, index);
                                }
                        });
                        
                        // Sort: latest episodes anime first (in order), then remaining alphabetically
                        results.sort((a, b) => {
                                const hasA = slugOrderMap.has(a.slug);
                                const hasB = slugOrderMap.has(b.slug);
                                
                                // Both are in latestEpisodes - sort by their order
                                if (hasA && hasB) {
                                        return slugOrderMap.get(a.slug) - slugOrderMap.get(b.slug);
                                }
                                // Only A is in latestEpisodes - A comes first
                                if (hasA) return -1;
                                // Only B is in latestEpisodes - B comes first
                                if (hasB) return 1;
                                // Neither in latestEpisodes - sort alphabetically
